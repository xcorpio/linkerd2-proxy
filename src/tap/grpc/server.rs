...
impl<T: Subscribe<Tap> + Clone> api::server::Tap for Server<T> {
    ...
    fn observe(&mut self, req: grpc::Request<api::ObserveRequest>) -> Self::ObserveFuture {
        ...
        let tap = Arc::new(Tap {
            match_,
            tx: Some(tx),
            remaining: Arc::new(AtomicUsize::new(req.limit as usize)),
        });
...
impl tap::Tap for Tap {
    ...
    fn tap<B: Payload>(&self, req: &http::Request<B>) -> Option<(TapRequestBody, TapResponse)> {
        if !self.match_.matches(&req) {
            return None;
        }

        loop {
            let mut tx = match self.tx.as_mut() {
                Some(tx) => tx,
                None => return None,
            };

            // Determine if there are more slots remaining without locking.
            //
            // `AtomicUsize::fetch_sub` cannot be used because it wraps.
            ...
            // Claim a tap slot (without locking).
            ...
            return tx.try_send(req_open(req)).ok().map(|()| {
                let req = TapRequestBody(tx.clone());
                let rsp = TapResponse(tx.clone());
                (req, rsp)
            });
        }
    }
