//! A stack module that lazily, dynamically resolves a `DstAddr` target, via DNS,
//! to determine it's canonical fully qualified domain name.
//!
//! For example, an application may set an authorithy value like `web:8080` with a
//! resolv.conf(5) search path of `example.com example.net` -- this module binds
//! its inner stack with a `CanonicalDstADdr`
